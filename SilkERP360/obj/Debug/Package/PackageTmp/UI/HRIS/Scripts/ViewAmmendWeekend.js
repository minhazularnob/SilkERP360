$(document).ready(function () {
    ...
    $("#txtWeekendDate").datepicker({ dateFormat: 'dd/MM/yy', changeMonth: true, changeYear: true, showButtonPanel: true, minDate: "+1D", maxDate: "+365D" });
    ...
    $('#tblEmployees').appendGrid({
        ...
        initRows: 0,
        columns: [
                    {name: 'txtWeekendCode', display: 'Weekend Code', type: 'text', displayCss: { 'width': '10%', 'text-align': 'center' }, ctrlCss: { 'width': '100%', 'text-align': 'center'} },
                    ...
                    { name: 'txtEmployeeName', display: 'Emp. Name', type: 'text', value: '', displayCss: { 'width': '30%', 'text-align': 'center' }, ctrlCss: { 'width': '100%', 'text-align': 'center' }, ctrlClass: 'peel_off_ip_required' },
                    ...
                    { name: 'txtDesignation', display: 'Degn.', type: 'text', value: '', displayCss: { 'width': '20%', 'text-align': 'center' }, ctrlCss: { 'width': '100%', 'text-align': 'center' }, ctrlClass: 'peel_off_ip_required' },
                    { name: 'txtWeekendDay', display: 'Day', type: 'text', value: '', displayCss: { 'width': '10%', 'text-align': 'center' }, ctrlCss: { 'width': '100%', 'text-align': 'center' }, ctrlClass: 'peel_off_ip_required' },
        ...
        customRowButtons: [
            {
                uiButton: { icons: { primary: 'ui-icon-cancel' }, text: false },
                click: function (evtObj, uniqueIndex, rowData) {
                    var lcl_str_EntryEmployeeCode = $('#txtSignedInEmployeeCode').val();
                    var lcl_str_WeekendCode = rowData["txtWeekendCode"]; // $("#txtWGOperationMasterCode").val();

                    var options = {};
                    options.url = gbl_URL_Root + "WebServices/HRIS/WeekendManagementService.asmx/CancelWeekend";
                    options.dataType = "json";
                    options.type = "POST";
                    options.data = "{IP_ui64_WeekendCode : " + lcl_str_WeekendCode + "}"; // JSON.stringify(lcl_obj_LogFile);
                    ...
                    options.success = function (result) {
                        ...
                        if (lcl_obj_WSResponse.ResponseCode == 0) {
                            ...
                            $('#tblEmployees').appendGrid('removeRow', (uniqueIndex-1));
                        }
                    };
                ...
                btnAttr: { title: 'Cancel Weekend' }, atTheFront: true
            }
        ],
...
function GetWeekendList(event) {
    var lcl_ui64_CompanyCode = $("#ddlCompany option:selected").val();
    var lcl_dt_WeekendDate = $("#txtWeekendDate").val();
    //var lcl_dt_DateUpto = $("#txtUptoDate").val();
    if (lcl_dt_WeekendDate == '') {
        DisplayError("The Field 'WeekendDate' must be filled");
        return;
    }
    $.ajax(
        {
            ...
            url: "~/../../../WebServices/HRIS/WeekendManagementService.asmx/GetWeekendEmployeesByDate",
            //url: gbl_URL_Root + "WebServices/HRIS/EmployeeService.asmx/GetRoosterAvailableEmployeeProfileListByDepartment",
            data: "{IP_dt_WeekendDate:" + JSON.stringify(lcl_dt_WeekendDate) + "}", //provide input for the getSM_PO method
            ...
            success: function (response) {
                ...
                var lcl_obj_EmployeeWithWeekendList = WSReturn.Data;
                //CLEAR Ref Employee
                ...
                $.each(lcl_obj_EmployeeWithWeekendList, function (index, lcl_obj_EmployeeWithWeekend) {
                    $('#tblEmployees').appendGrid('appendRow', [
                    { txtWeekendCode: lcl_obj_EmployeeWithWeekend.WeekendCode.toString(),
                        txtEmployeeId: lcl_obj_EmployeeWithWeekend.EmployeeId.toString(),
                        txtEmployeeName: lcl_obj_EmployeeWithWeekend.EmployeeName.toString(),
                        txtDesignation: lcl_obj_EmployeeWithWeekend.DesignationName.toString(),
                        txtDepartment: lcl_obj_EmployeeWithWeekend.DepartmentName.toString(),
                        txtWeekendDay: lcl_obj_EmployeeWithWeekend.WeekDayNameSTR.toString()
                    }]);
