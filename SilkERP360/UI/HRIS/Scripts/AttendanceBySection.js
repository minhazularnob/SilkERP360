...
$(document).ready(function () {
    ...
    initializeSelect2('ddlSection', '------ Select Designation ------', '25%');
});

function GetAttendanceByDesignationListAndDate(event) {
    ...
    $.ajax(
    {
        ...
        global: true,
        url: gbl_URL_Root + "WebServices/HRIS/AttendanceService.asmx/GetAttendanceByDesignationListAndDate",
        data: "{IP_enm_Section: " + lcl_ui64_SectionCode + ",IP_dt_AttendanceDate:'" + lcl_dt_AttendanceDate + "'}", //provide input for the getSM_PO method
        dataType: "json",
        success: function (response) {
            ...
            if (WSResponse.ResponseCode == -100) {
                ...
                var lcl_i32_Count = $('#tblSectionEmployee').appendGrid('getRowCount');
                for (var i = 0; i < lcl_i32_Count; i++) {
                    $('#tblSectionEmployee').appendGrid('removeRow', 0);
                }
                return;
            }
...
function DisplayAttendanceMaster() {
    $("#txtTotalEmployee").val(GBLAttendanceMaster.TotalProcessed);
    ...
    if (GBLAttendanceMaster.AttendanceList != null) {
        ...
        $.each(GBLAttendanceMaster.AttendanceList, function (index, lcl_obj_Attendance) {
            ...
            $('#tblSectionEmployee').appendGrid('appendRow', [
