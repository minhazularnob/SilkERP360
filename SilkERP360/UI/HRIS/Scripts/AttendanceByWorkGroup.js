...
$(document).ready(function () {
    ...
    initializeSelect2('ddlWorkGroup', '------ Select Employee ------', '25%');
});

function GetAttendanceByWorkGroupDate(event) {
    ...
    $.ajax(
    {
        ...
        global: true,
        url: gbl_URL_Root + "WebServices/HRIS/AttendanceService.asmx/GetAttendanceByGroupAndDate",
        data: "{IP_ui64_WorkGroupCode: " + lcl_ui64_WorkGroupCode + ",IP_dt_AttendanceDate:'" + lcl_dt_AttendanceDate + "'}",
        dataType: "json",
        success: function (response) {
            ...
            if (WSResponse.ResponseCode == -100) {
                ...
                var lcl_i32_Count = $('#tblWorkgroupAttendance').appendGrid('getRowCount');
                for (var i = 0; i < lcl_i32_Count; i++) {
                    $('#tblWorkgroupAttendance').appendGrid('removeRow', 0);
                }
                return;
            }
...
function DisplayAttendanceMaster() {
    ...
    if (GBLAttendanceMaster.AttendanceList != null) {
        ...
        $.each(GBLAttendanceMaster.AttendanceList, function (index, lcl_obj_Attendance) {
            ...
            $('#tblWorkgroupAttendance').appendGrid('appendRow', [
...
function ProcessAttendanceForWorkGroupByDate(event) {
    event.preventDefault();
    var lcl_ui64_WorkGroupCode = $("#ddlWorkGroup option:selected").val();
    var lcl_dt_AttendanceDate = $("#txtAttendanceDate").val();
    ...
    $.ajax(
    {
        ...
        contentType: "application/json; charset=utf-8",
        global: true,
        url: gbl_URL_Root + "WebServices/HRIS/AttendanceService.asmx/ProcessAttendanceByGroupAndDate",
        data: "{IP_ui64_WorkGroupCode: " + lcl_ui64_WorkGroupCode + ",IP_dt_AttendanceDate:'" + lcl_dt_AttendanceDate + "'}",
        dataType: "json",
        success: function (response) {
            var WSResponse = response.d;
            ...
            if (WSResponse.ResponseCode == -1) {
                ShowInfoMessageBoard("Attendance of Selected WorkGroup for the selected date has not yet been processed!!!");
                return;
            }
            if (WSResponse.ResponseCode == 0) {
