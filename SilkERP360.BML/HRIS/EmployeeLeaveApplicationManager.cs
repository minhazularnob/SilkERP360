...
namespace SilkERP360.BML.HRIS
{
     ...
     SilkERP360.CCL.Interfaces.IManagerOperations<SilkERP360.CCL.BusinessEntities.HRIS.EmployeeLeaveApplication>
    {
        ...
        public static void GetEffectiveMonthYear(DateTime inputDate, out int effectiveMonth, out int effectiveYear)
        {
            if (inputDate.Day >= 26)
            {
                DateTime nextMonth = inputDate.AddMonths(1);
                effectiveMonth = nextMonth.Month;
                effectiveYear = nextMonth.Year;
            }
            else
            {
                effectiveMonth = inputDate.Month;
                effectiveYear = inputDate.Year;
            }
        }
        ...
        {
            System.UInt64 lcl_ui64_LeaveApplicationCode = 0;
            System.String lcl_str_SqlQuery = System.String.Format("SELECT max(leave_application_code)+1 as id from employee_leave_application");
            lcl_ui64_LeaveApplicationCode = this.ExceptionManager.Process<System.UInt64>(() =>
            {
                using (var lcl_obj_DBManager = SilkERP360.DAL.DALObjectPoolManager.DBManagerPool.GetObject())
                {
                    ...
                    else
                    {
                        ...
                        for (DateTime date = IP_obj_LeaveApplication.LeaveStartDate; date <= IP_obj_LeaveApplication.LeaveEndDate; date = date.AddDays(1))
                        {
                            int effectiveMonth;
                            int effectiveYear;

                            // 🔹 Salary cycle get month/year
                            GetEffectiveMonthYear(date, out effectiveMonth, out effectiveYear);

                            System.String lcl_str_SqlQuerys = System.String.Format("SELECT max(add_ded_code)+1 as id from SALARY_ADDITION_DEDUCTION");
                            if (lcl_obj_DBManager.InternalResource.ConnectionState != System.Data.ConnectionState.Open)
                            {
                                lcl_obj_DBManager.InternalResource.Open();
                            }
                            ...
                            Oracle.ManagedDataAccess.Client.OracleDataReader lcl_obj_IDReaders = lcl_obj_DBManager.InternalResource.ExecuteDataReader(lcl_str_SqlQuerys);
                            lcl_obj_IDReaders.Read();
                            System.UInt64 lcl_ui64_addDedId = System.UInt64.Parse(lcl_obj_IDReaders["id"].ToString());
                            lcl_obj_IDReaders.Close();

                            string sql =
                                "INSERT INTO SALARY_ADDITION_DEDUCTION " +
                                "(ADD_DED_CODE, EMPLOYEE_CODE, ADD_OR_DED, ADD_DED_TYPE, AMOUNT, ADD_DED_DATE, REMARKS, EFFECTIVE_MONTH, EFFECTIVE_YEAR, IS_PROCESSED, STATUS, ENTRY_EMPLOYEE_CODE, ENTRY_DATE) " +
                                "VALUES (" + lcl_ui64_addDedId + ", " +
                                IP_obj_LeaveApplication.EmployeeCode + ", " +
                                (int)AdditionOrDeduction.Deduction + ", " +
                                (int)AdditionDeductionType.DeductionUnpaidLeave + ", " +
                                lcl_dcm_OneDayBasicSalary + ", " +
                                "SYSDATE, " +
                                "'Unpaid Leave Deduction', " +
                                effectiveMonth + ", " +
                                effectiveYear + ", " +
                                "0, 1, " + IP_obj_LeaveApplication.EntryEmployeeCode + ", SYSDATE)";

                            lcl_obj_DBManager.InternalResource.ExecuteNonQuery(sql);
                        }
                    }
